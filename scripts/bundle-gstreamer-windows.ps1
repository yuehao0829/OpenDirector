...
$GstRoot = if ($env:GENLINE_GSTREAMER_RUNTIME_ROOT) {
    $env:GENLINE_GSTREAMER_RUNTIME_ROOT
...
$RuntimeDir = if ($env:GENLINE_GSTREAMER_BUNDLE_RUNTIME_DIR) {
    $env:GENLINE_GSTREAMER_BUNDLE_RUNTIME_DIR
} else {
    Join-Path $PSScriptRoot "..\apps\desktop\src-tauri\gstreamer-runtime"
}
$RootDllDir = if ($env:GENLINE_GSTREAMER_BUNDLE_ROOT_DLL_DIR) {
    $env:GENLINE_GSTREAMER_BUNDLE_ROOT_DLL_DIR
} else {
