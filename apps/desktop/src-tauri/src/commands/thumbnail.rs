...
/// Decoded audio with sample rate information.
...
pub fn decode_audio_samples(path: &str) -> Result<DecodedAudio, String> {
...
pub fn decode_wav(path: &str) -> Result<DecodedAudio, String> {
    let reader =
...
pub fn decode_symphonia(path: &str) -> Result<DecodedAudio, String> {
...
pub fn downmix_to_mono(samples: Vec<f32>, channels: usize) -> Result<Vec<f32>, String> {
    if channels <= 1 {
        return Ok(samples);
    }
