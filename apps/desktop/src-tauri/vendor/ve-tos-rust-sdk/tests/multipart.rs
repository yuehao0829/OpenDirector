...
use ve_tos_rust_sdk::multipart::{
    ...
    ListMultipartUploadsInput, ListPartsInput, MultipartAPI, UploadPartCopyInput,
    UploadPartFromBufferInput, UploadPartFromFileInput, UploadPartInput, UploadedPart,
};
use ve_tos_rust_sdk::object::{GetObjectInput, ObjectAPI, ObjectContent};

use crate::common::{base64_md5, create_context, gen_random_string, read_to_buf, Context};
...
fn test_main() {
    ...
    test_multipart_list(&context);
    test_multipart_upload(&context);
}

fn test_multipart_upload(context: &Context) {
    ...
    if meta.is_err() {
        {
            ...
            for i in 0..500000 {
                fd.write(random_str.as_bytes()).unwrap();
            }
        }
        meta = fs::metadata(big_file_path);
    }
    let file_size = meta.unwrap().size();
    let source_md5 = base64_md5(read_to_buf(&mut File::open(big_file_path).unwrap()));
    println!("{source_md5}");

    let first_part_size = 5 * 1024 * 1024;
    let mut etags = Vec::with_capacity(2);
    {
        let mut input = UploadPartInput::<File>::new(bucket, key, upload_id);
        input.set_part_number(1);
        let fd = File::open(big_file_path).unwrap();
        input.set_content(fd);
        input.set_content_length(first_part_size);
        let o = client.upload_part(&input).unwrap();
        assert!(o.request_id().len() > 0);
    ...
    }
    {
        ...
        let mut fd = File::open(big_file_path).unwrap();
        fd.seek(SeekFrom::Start(first_part_size as u64)).unwrap();
        input.set_content(fd);
    ...
    let mut o = client
        ...
        .unwrap();
    assert!(o.request_id().len() > 0);
    let target_md5 = base64_md5(read_to_buf(o.content().unwrap()));
    ...
    let o = client.upload_part_from_file(&input).unwrap();
    assert!(o.request_id().len() > 0);
    println!("upload part etag1 done, {}", o.etag());
    etags.push(o.etag().to_string());
    let mut input = UploadPartFromFileInput::new(bucket, key2, upload_id2);
    ...
    let o = client.upload_part_copy(&input).unwrap();
    assert!(o.request_id().len() > 0);
...
fn test_multipart_list(context: &Context) {
    let client = context.client();
    ...
    loop {
        let o = client.list_multipart_uploads(&input).unwrap();
