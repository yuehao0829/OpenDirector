...
fn on_event_loop_event<R: Runtime>(
  ...
  let event = match event {
    ...
    RuntimeRunEvent::Ready => {
      // OpenDirector workaround:
      // Tauri's macOS dev-only icon path crashes inside NSImage parsing on some systems.
      // Keep the runtime behavior unchanged on other targets and in bundled builds.
